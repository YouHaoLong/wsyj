...
var stepBar = {
    ...
    _animate : function(){
        ...
        if(this.triggerStep < this.curStep){
            ...
            stepSequence_size.padding = "8px 16px";
...
$('.fold').on('click',function(){ //点击现场指挥中心的通讯录图标改变指向联系人的图标
	var class1=$(this).find('i').attr('class');
	if(class1.indexOf('icon-angle_d')==-1){
	  $(this).find('i').removeClass('icon-angle_r').addClass('icon-angle_d');
	}else {
	  $(this).find('i').removeClass('icon-angle_d').addClass('icon-angle_r');
	}

   var index=$('.fold').index(this); //点击联系人的图标，显示与隐藏下一级联系人
   $('.fold_sub').eq(index).slideToggle(500);
})

$('.ui-stepSequence').on('click',function(){//点击流程的每一个选项分别切换到不同的页面
	var index=$('.ui-stepSequence').index(this); //点击各模块会切换到相应的编辑页面
    $('.step-list').eq(index).removeClass('dis').siblings().addClass('dis');
})

//选项卡切换
$('.tab_item li').on('click',function(){
	$(this).addClass('ac').siblings().removeClass('ac');
	$('.tab_content').eq($(this).index()).removeClass('dis').siblings().addClass('dis');
})